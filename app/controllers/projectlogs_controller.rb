class ProjectlogsController < ApplicationController
  ...
  def index
    ...
    @projectlogs = Projectlog.where('project_id=?', params[:project_id]).all
    #@projectlogs = Projectlog.find_by_sql("select l.* from projectlog l where l.project_id = "+params[:project_id])　
  ...
  private
    ...
    def projectlog_params
      params.require(:projectlog).permit(:action, :actiondata, :username)
    end
end
